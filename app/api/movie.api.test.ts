...
describe("searchMovies", () => {
    ...
    it("returns the parsed response on success", async () => {
        const mockResponse = { Search: [{ Title: "Inception" }], totalResults: "1", Response: "True" };
...
describe("getMovieDetails", () => {
    ...
    it("fetches and returns movie details", async () => {
        const mockMovie = { imdbID: "tt1375666", Title: "Inception", Response: "True" };
    ...
    it("serves the second request from cache without hitting fetch again", async () => {
        const mockMovie = { imdbID: "tt1375667", Title: "The Matrix", Response: "True" };
        (fetch as any).mockResolvedValue({ json: async () => mockMovie });
    ...
    it("dedupes concurrent calls for the same imdbID into a single fetch", async () => {
        ...
        resolveFetch!({ json: async () => ({ imdbID: "tt1375668", Title: "Dunkirk", Response: "True" }) });
    ...
    it("throws using OMDb's own error message when Response is False", async () => {
        ...
        await expect(getMovieDetails("tt0000000")).rejects.toThrow("Incorrect IMDb ID.");
    });
    ...
    it("falls back to a generic message if OMDb omits Error", async () => {
        ...
        await expect(getMovieDetails("tt0000001")).rejects.toThrow("Failed to fetch Movie details");
    });

    it("clears the in-flight entry after a failure, so a retry actually refetches", async () => {
        (fetch as any)
            .mockResolvedValueOnce({ json: async () => ({ Response: "False", Error: "boom" }) })
            .mockResolvedValueOnce({ json: async () => ({ imdbID: "tt0000002", Title: "Retry", Response: "True" }) });
