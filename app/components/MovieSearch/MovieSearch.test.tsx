...
vi.mock("../../Common/MovieCard", () => ({
    MovieCard: ({ movie }: any) => <div data-testid={`movie-card-${movie.imdbID}`}>{movie.Title}</div>,
}));
...
describe("MovieSearch", () => {
    ...
    it("shows the 'no results' state when the API returns nothing", async () => {
        (searchMovies as any).mockResolvedValue({ Response: "True", totalResults: "0", Search: [] });
    ...
    it("clears results immediately on each keystroke, before the new search resolves", async () => {
        ...
        (searchMovies as any).mockReturnValue(new Promise((resolve) => { resolveSearch = resolve; }));
        ...
        await waitFor(() => expect(screen.getByTestId("movie-card-tt1")).toBeInTheDocument());
