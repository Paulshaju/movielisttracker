...
export function loadInitialState() {
  if (hasHydrated)
    return { playlists: movieStore.playlists, tags: movieStore.tags };
  hasHydrated = true;
  const defaultPlaylists = () =>
    proxyMap<string, IPlaylistListItem[]>([
      ["watched", []],
      ["watchlist", []],
    ]);
...
export const getList = (key: string): IPlaylistListItem[] =>
  movieStore.playlists.get(key) ?? [];
...
export const addToWatchlist = (newMovieItem: IPlaylistListItem) => {
  ...
  setList("watchlist", [
    ...watchlist,
    { ...newMovieItem, addedAt: new Date().toDateString() },
  ]);
};
...
export const removeFromWatchlist = (movieId: string) => {
  ...
  setList(
    "watchlist",
    watchlist.filter((elm) => elm.imdbID !== movieId),
  );
};
...
export const moveToWatched = (
  ...
  setList("watched", [
    ...watched,
    { ...movieItem, userRating, note, addedAt: new Date().toDateString() },
  ]);
  setList(
    "watchlist",
    watchlist.filter((elm) => elm.imdbID !== movieId),
...
export const removeFromWatched = (movieId: string) => {
  ...
  setList(
    "watched",
    watched.filter((elm) => elm.imdbID !== movieId),
  );
};
...
export const addToList = (
  listName: string,
  newMovieItem: IPlaylistListItem,
) => {
...
export const removeFromList = (listName: string, movieId: string) => {
  ...
  setList(
    listName,
    list.filter((elm) => elm.imdbID !== movieId),
  );
};
...
export const renameList = (oldName: string, newName: string) => {
  ...
  if (!movieStore.playlists.has(oldName) || movieStore.playlists.has(newName))
    return;
